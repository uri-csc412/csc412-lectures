...
int main(int argc, char **argv) {
    ...
    // small buffer for educational purposes said the dark wizard
    char buffer[16]; 

    // check if mmap failed to get memory
    if (shared_memory == MAP_FAILED) {
        perror("mmap");
        return 1;
    }
    
    // get process id, it should not be -1
    pid_t pid = fork();
    if (pid == -1) {
        perror("fork");
        return 1;
        ...
        *shared_memory = 5;
        ...
        /*
            we are inetionally cause a buffer overflow here. This is an unsafe use of the gets() function, which does not check buffer boundaries.
